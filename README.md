...
2. **Tạo nhánh riêng cho công việc của bạn:**
   _(Ví dụ: bạn được giao nhiệm vụ viết API hiển thị sản phẩm)_
   ```bash
...
### A. Chạy riêng MySQL giống ảnh (mysql:8.0.45-debian)

```bash
docker pull mysql:8.0.45-debian
docker run -d --name mysql-8.0.45 -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=ProFitSuppsDB -v profit_mysql_data:/var/lib/mysql mysql:8.0.45-debian
```

### B. Chạy full stack (MySQL + Backend + Frontend)

Tại thư mục gốc `ProFit/`:

```bash
docker compose up -d --build
```

Compose sẽ dùng MySQL đã chạy sẵn là `mysql-8.0.45` trên cổng `3306`.

Các địa chỉ sau khi chạy:

- Frontend: `http://localhost:5173`
- Backend: `http://localhost:8080/ProFitSuppsDB`
- MySQL: `localhost:3306` (container `mysql-8.0.45`)

### C. Chạy thêm Chatbot (tùy chọn)
...
- Service `chatbot` cần `OPENAI_API_KEY` nếu muốn gọi LLM thật.
- Database được dùng từ container `mysql-8.0.45` chạy độc lập trên cổng `3306`.
